...
namespace Prion
{
    public class PrionUBigInt : PrionNode
    {
        ...
        public PrionUBigInt() : base(PrionType.UBigInt){}
        ...
        public static new bool TryFromString(string value, out PrionNode node)
        {
            ...
            {
                node = new PrionError($"ubigint signature not present at start of string {value}.");
                return false;
            }
            value = value[8..].Trim();
            if(value.StartsWith("0x") && TryFromHexString(value, out node)) return true;
            if(value.StartsWith("0b") && TryFromBinaryString(value, out node)) return true;
            node = new PrionError($"Invalid ubigint value {value}");
            return false;
        }
        ...
        public static bool TryFromBinaryString(string value, out PrionNode node)
        {
            ...
            for (int idx = 0; idx < value.Length; idx++)
            {
                ...
                {
                    node = new PrionError($"Found invalid char {c} in binary string");
                    return false;
                }
            }
            node = res;
